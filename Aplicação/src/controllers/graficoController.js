...
function buscarJogadoresPontuacoesComplete(req, res) {
    graficoModel.buscarJogadoresPontuacoesComplete()
        .then(resultados => {
            res.json(resultados);
        })
        .catch(erro => {
...
function buscarMelhoresPontuadoresComplete(req, res) {
    graficoModel.buscarMelhoresPontuadoresComplete()
        .then(resultados => {
            if (resultados.length > 0) {
                res.status(200).json(resultados);
            } else {
